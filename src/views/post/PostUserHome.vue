...
const activeTab = ref(0); // 当前选中的选项卡，0=草稿，1=已发送
const router = useRouter();
const defaultPicture = "https://gitee.com/Code_for_love/images/raw/master/-6696978110499230174test.png";

// 请求接口返回的数据
const fetchPosts = async (visible, current, pageSize) => {
  const userInfo = await userStore.getUserInfo();
  const postOwnRequest = {
    current: current,
    pageSize: pageSize,
    sortField: '',
    userId: userInfo.id,
  ...
  const resp = await axios.post('/api/post/listOwnPosts', postOwnRequest);
  const {data} = resp;
  const {records} = data;
  // console.log(records);
  return records;
...
const onLoad = async () => {
  ...
  try {
    const newPosts = await fetchPosts(activeTab.value, current.value, pageSize);
